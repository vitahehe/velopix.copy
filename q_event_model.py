...
class segment:
   ...
   def __post_init__(self):
       object.__setattr__(self, 'truth', (self.from_hit.id == self.to_hit.id) and (self.from_hit.id is not None) and ((self.to_hit.id is not None)))

   def to_vect(self):
       return np.array([self.to_hit.x - self.from_hit.x, self.to_hit.y - self.from_hit.y, self.to_hit.z - self.from_hit.z])

   def display(self, ax, equal_axis = True, c='black'):
       ax.plot((self.from_hit.x, self.to_hit.x),(self.from_hit.y, self.to_hit.y),(self.from_hit.z, self.to_hit.z),lw=.1,c=c)
